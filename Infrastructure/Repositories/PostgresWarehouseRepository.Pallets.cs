...
{
    private async Task<PalletRecord?> FindBestOpenPalletAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string productNumber, string expiryDate, CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText = """
            ...
            ORDER BY HasExact DESC, CAST(SUBSTRING(p.PalletId FROM 3) AS INTEGER), p.PalletId;
            """;
    ...
    private static async Task<int> GetNextPalletNumberAsync(NpgsqlConnection connection, NpgsqlTransaction tx, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
        // Numeric suffix keeps pallet IDs monotonic and human-friendly (P-001, P-002, ...).
        cmd.CommandText = """
            SELECT COALESCE(MAX(CAST(SUBSTRING(PalletId FROM 3) AS INTEGER)), 0)
            FROM Pallets;
            """;
    ...
    private static async Task InsertPalletAsync(NpgsqlConnection connection, NpgsqlTransaction tx, PalletRecord pallet, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = """
        ...
        cmd.Parameters.AddWithValue("@closed", pallet.IsClosed);
        cmd.Parameters.AddWithValue("@created", pallet.CreatedAt);
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }

    private static async Task UpsertPalletItemAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string palletId, string productNumber, string expiryDate, int quantity, CancellationToken cancellationToken)
    {
        ...
        insert.CommandText = """
            ...
            ON CONFLICT (PalletId, ProductNumber, ExpiryDate)
            DO UPDATE SET Quantity = PalletItems.Quantity + EXCLUDED.Quantity;
            """;
    ...
    private static async Task DecreaseOrDeletePalletItemAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string palletId, string productNumber, string expiryDate, int quantity, CancellationToken cancellationToken)
    {
    ...
    private static async Task UpdatePalletQuantityAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string palletId, int quantity, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
    ...
    private static async Task<PalletRecord?> GetPalletByIdAsync(NpgsqlConnection connection, NpgsqlTransaction? tx, string palletId, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = """
    ...
    private static async Task DeletePalletItemsAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string palletId, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
    ...
    private static async Task DeletePalletAsync(NpgsqlConnection connection, NpgsqlTransaction tx, string palletId, CancellationToken cancellationToken)
    {
        await using var cmd = connection.CreateCommand();
        cmd.Transaction = tx;
